...
with tf.Session(graph=graph) as session:
    ...
    stats_net = session.run(
        ...
        feed_dict={x: get_batch('training', range(train_sample)),
                   t: toxicity['training'][:train_sample],
                   keep_prob: 1.0}
    )
    ...
    for epoch in range(args.num_epochs):
        for rotation in args.rotations:
            ...
            # TRAIN #
            x_t, y_t = shuffle(range(ds_sizes['training']), toxicity['training'])
            ...
            stats_v = session.run(
                ...
                feed_dict={x: get_batch('validation', range(val_sample)),
                           t: toxicity['validation'][:val_sample],
                           keep_prob: 1.0}
            )
