...
#Addition 
arc_group.add_argument('--dense_sizes_2', metavar='DD', default=[100],
                       type=int, nargs='+',
                       help='number of neurons in additional dense layers')
...
for dictionary in [ids, affinity, toxicity, coords, features]:
    for dataset_name in datasets:
...
for dataset_name in datasets:
    ...
    with h5py.File(dataset_path, 'r') as f:
        for pdb_id in f:
            ...
            toxicity[dataset_name].append(dataset.attrs['toxicity'])
    ...
    toxicity[dataset_name] = np.reshape(toxicity[dataset_name], (-1, 1))
...
print('baseline mse for affinity: training=%s, validation=%s' % (t_baseline, v_baseline))

# Best error we can get without any training (MSE from training set mean):
t2_baseline = ((toxicity['training'] - toxicity['training'].mean()) ** 2.0).mean()
v2_baseline = ((toxicity['validation'] - toxicity['training'].mean()) ** 2.0).mean()
print('baseline mse for toxicity: training=%s, validation=%s' % (t2_baseline, v2_baseline))
...
print('fully connected layers:', ', '.join((str(i) for i in args.dense_sizes)),
      'neurons')
print('additional fully connected layers for predicting toxicity:', ', '.join((str(i) for i in args.dense_sizes_2)),
      'neurons')
...
graph = tfbio.net.make_SB_network(isize=isize, in_chnls=in_chnls, osize=osize,
                                  ...
                                  dense_sizes_2=args.dense_sizes_2,
                                  lmbda=args.lmbda,
...
y_2 = graph.get_tensor_by_name('output/prediction_2:0') #Addition
t = graph.get_tensor_by_name('input/affinity:0')
t_2 = graph.get_tensor_by_name('input/toxicity:0') #Addition
...
mse_2 = graph.get_tensor_by_name('training/mse_2:0')
...
fcs_2 = '_'.join((str(i) for i in args.dense_sizes_2))
