...
"""
Usange Example 
python prepare_3.py -l dataset/ligand_/train/*.sdf --ligand_format sdf 
-p dataset/protein/train/*.pdb --pocket_format pdb -t dataset/toxicities/exp_toxicities.csv 
-o dataset/train_valid_test_data/training_set.hdf
"""
...
parser = argparse.ArgumentParser(
    ...
    It also saves toxicity values as attributes, if they are provided.
    ...
    and saves all required data to predict toxicity for each molecular complex.
...
parser.add_argument('--toxicities', '-t', default=None, type=input_file,
                    help='CSV table with toxicity values.'
                         ' It must contain two columns: `name` which must be'
                         ' equal to ligand\'s file name without extenstion,'
                         ' and `toxicity` which must contain floats')
...
# TODO: training set preparation (allow to read toxicities)
...
if args.toxicities is not None:
    toxicities = pd.read_csv(args.toxicities)
    if 'toxicity' not in toxicities.columns:
        raise ValueError('There is no `toxicity` column in the table')
    elif 'name' not in toxicities.columns:
        raise ValueError('There is no `name` column in the table')
    toxicities = toxicities.set_index('name')['toxicity']
else:
    toxicities = None
    print('#'*10,'\ntoxicity value is empty. you have to put your toxicity data.\n','#'*10) #Addition
...
with h5py.File(args.output, args.mode) as f:
    ...
    for ligand_file in args.ligand:
        ...
        if toxicities is not None:
            dataset.attrs['toxicity'] = toxicities.loc[name]
if args.verbose:
